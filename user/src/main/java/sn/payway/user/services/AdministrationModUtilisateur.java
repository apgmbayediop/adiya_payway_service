...
import sn.payway.user.object.APGUtilisateur;
import sn.payway.user.object.APGUtilisateurResponse;
		...
		try {
			...
			if(apgUtilisateurRequest.getIsDongle() != null && apgUtilisateurRequest.getIsDongle()) {
				...
				if(!dongleSession.bindUser(apgUtilisateurRequest.getReference(), user)){
					utilVue.CommonObject(null, TAG, ErrorResponse.SYNTAXE_ERRORS_1709.getCode(), ErrorResponse.SYNTAXE_ERRORS_1709.getMessage("Utilisateur not bind"), user);
				}
			}else {
				if(!dongleSession.unbindUser(user)){
					utilVue.CommonObject(null, TAG, ErrorResponse.SYNTAXE_ERRORS_1709.getCode(), ErrorResponse.SYNTAXE_ERRORS_1709.getMessage("Utilisateur not unbind"), user);
				}
			}
