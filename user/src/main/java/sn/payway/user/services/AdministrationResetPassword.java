...
public class AdministrationResetPassword {
	...
	public Response Service(String flashcode, APGCommonRequest apgResetPasswordRequest){
		try {
			...
			String opt = "";
			if(user.getPartner().getCode().equals(BEConstantes.CODE_OPTIMA_PROVIDER)  || (user.getIdGroupeUtilisateur().equals(BEConstantes.GROUPE_PARTENAIRE_ECOMMERCE) && user.getPartner().getParent().getCode().equals(BEConstantes.CODE_OPTIMA_MONETIQUE))) {
