...
@Singleton
public class BrmGimController {

	private static final Logger LOG = Logger.getLogger(BrmGimController.class);
	...
	private static final String ID_BANQUE = "BRM";
	...
	@PostConstruct
	public void init() {
		LOG.info("init brm gim controller");
		String sep = File.separator;
		template = BEConstantes.path + sep + "config" + sep + "isoascii.xml";
		LOG.info("template " + template);
		Session session = (Session) BeanLocator.lookUp(SessionBean.class.getSimpleName());
		ParametresGeneraux param2 = session.findObjectById(ParametresGeneraux.class, null, "PARAM_SWITCH_APG");
		if (param2 == null) {
	...
	private void notifyFailConnexion() {
		try {
			...
			//mailSender.sendSimpleMail("GIM interface non disponible " + props.getProperty(PaymentUtils.ENV_PRPOPERTY), body,mailNotify);
		} catch (Exception e) {
	...
	private void restablishConnexion() {
		try {
			...
			if (failedTime != 0) {
				//String object = "GIM retablissement connexion " + props.getProperty(PaymentUtils.ENV_PRPOPERTY);
				//String body = "retablissement connexion interface " + ID_BANQUE + " GIM aprés " + failedTime + " mn";
				//mailSender.sendSimpleMail(object, body, mailNotify);
				failedTime = 0;
			}
		} catch (Exception e) {
