...
import sn.payway.payment.dto.CommissionDto;
...
public class OnlinePaymentService {
	...
	public boolean autorizeServer(PaymentDto dto) {
		String calAuth = new StringBuilder().append(dto.getTimestamp()).append(dto.getTransactionId())
				.append(config.getProperty("payway.key")).toString();
	...
	public PaymentDetails initiateOnlinePayment(PaymentDto request) throws PaywayException{
			...
			if (check.getCode().equals(ErrorResponse.REPONSE_SUCCESS.getCode())) {
				...
				if ((ChannelResponse.CARD2W.getCode().equals(request.getTransactionType())
					...
					if(!verifyInitiatePay(request, null, caisse.getTerminalKey())) {
						throw new PaywayException(ErrorResponse.PARTNER_NOT_ALLOWED.getCode(),ErrorResponse.PARTNER_NOT_ALLOWED.getMessage(""));
					}
					...
					if (commercant.getCurrencyName().equals(request.getCurrencyName())) {
						...
						if (nextBalance.compareTo(account.getMaxBalance()) > 0) {
							throw new PaywayException(ErrorResponse.TRANSACTION_NOT_ALLOW_OPTIMA.getCode(),
						...
						if (commMonetique == null || commMonetique.isEmpty()) {
							...
							throw new PaywayException(ErrorResponse.TRANSACTION_CHANNEL_ERROR.getCode(),
						...
						if (findAcqui != null) {
							throw new PaywayException(ErrorResponse.TRANSACTION_DOUBLOON_ERROR.getCode(),
						...
						payUrl.append(config.getProperty("payway.baseUrl")).append("?transactionId=")
	...
	private boolean verifyInitiatePay(PaymentDto request, String flashcode, String terminalKey) throws PaywayException {
